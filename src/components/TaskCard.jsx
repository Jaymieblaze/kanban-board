...
function TaskCard({ task, deleteTask, updateTask }) {
      ...
      <>
        ...
        {createPortal(
          <>
            ...
            {/* Modal */}
            <div
              className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[9999] bg-white dark:bg-gray-800 p-4 sm:p-6 w-[95vw] sm:w-[500px] max-w-[90vw] rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto border border-purple-100 dark:border-gray-700"
            >
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-lg font-bold bg-gradient-to-r from-purple-600 to-indigo-600 dark:from-purple-400 dark:to-indigo-400 bg-clip-text text-transparent">Edit Task</h3>
            <button
              onClick={toggleEditMode}
              className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg p-2 transition-all touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center"
              aria-label="Close"
            >
              <X size={20} />
            </button>
          ...
          <div className="mb-4">
            ...
            <input
              type="text"
              className="w-full border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:border-purple-400 dark:focus:border-purple-500 focus:ring-4 focus:ring-purple-100 dark:focus:ring-purple-900 focus:outline-none transition-all"
              value={task.content}
          ...
          <div className="mb-4">
            ...
            <textarea
              className="w-full border-2 border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg px-3 py-2 focus:border-purple-400 dark:focus:border-purple-500 focus:ring-4 focus:ring-purple-100 dark:focus:ring-purple-900 focus:outline-none transition-all resize-none"
              rows="3"
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {task.tags && task.tags.map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center gap-1 bg-gradient-to-r from-purple-100 to-indigo-100 dark:from-purple-900/50 dark:to-indigo-900/50 text-purple-700 dark:text-purple-300 px-3 py-1 rounded-full text-sm font-medium shadow-sm"
                >
                  {tag}
                  <button
            ...
            <div className="flex gap-2">
              <input
                type="text"
                className="flex-1 border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:border-purple-400 dark:focus:border-purple-500 focus:ring-4 focus:ring-purple-100 dark:focus:ring-purple-900/50 focus:outline-none transition-all"
                value={newTag}
  ...
  return (
    <div
      ...
      {...listeners}
      onClick={toggleEditMode}
      className={`p-3 min-h-[120px] flex flex-col text-left rounded-lg hover:ring-2 hover:ring-purple-400 dark:hover:ring-purple-600 cursor-grab relative border-l-4 ${colors.border} bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-all`}
    ...
    >
      ...
      {/* Task Title */}
      <h4 className="font-medium text-gray-800 dark:text-gray-200 mb-2 line-clamp-2">
        {task.content}
      </h4>
      ...
      {task.tags && task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {task.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded text-xs"
            >
              <Tag size={10} />
              {tag}
            </span>
      ...
      {task.dueDate && (
        <div className={`flex items-center gap-1 text-xs mt-auto ${
          isOverdue(task.dueDate) ? "text-red-600 dark:text-red-400 font-medium" : "text-gray-500 dark:text-gray-400"
        }`}>
